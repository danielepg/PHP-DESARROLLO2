<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$query = mysqli_query($conn,"SELECT f.nofactura, DATE_FORMAT(f.fecha, '%d/%m/%Y') as fecha, DATE_FORMAT(f.fecha,'%H:%i:%s') as  hora, f.codcliente, 
												 cl.nit, cl.nombre, cl.telefono,cl.direccion
											FROM factura f											
											INNER JOIN cliente cl
											ON f.codcliente = cl.ID_CLIENTE
											WHERE f.nofactura = $noFactura AND f.codcliente = $codCliente");
